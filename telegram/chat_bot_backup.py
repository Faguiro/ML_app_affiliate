...
class ChatBot:
    ...
    async def send_photo(self, chat_id, photo_url, caption=None, parse_mode=None, as_bot=True):
        ...
        try:
            ...
            # Escolhe cliente (bot ou user)
            client = self.bot_client if as_bot else self.user_client
            
            print(f"📸 Baixando imagem: {photo_url[:80]}...")
            
            # Baixa a imagem
            async with aiohttp.ClientSession() as session:
                async with session.get(photo_url) as response:
                    if response.status != 200:
                        print(f"❌ Falha ao baixar imagem: HTTP {response.status}")
                        return False
            ...
            result = await client.send_file(
                ...
                caption=caption[:1024] if caption else None,  # Telegram limita a 1024 chars
