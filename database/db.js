...
class DatabaseManager {
    ...
    initDatabase() {
        try {
            ...
            if (fs.existsSync(schemaPath)) {
                ...
                this.db.exec(schema);
                this.db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_original_url ON tracked_links(original_url)`);
    ...
    canSendToGroup(groupJid) {
        ...
        console.log(`   sent_today: ${group.sent_today}, daily_limit: ${group.daily_limit}`);
        console.log(`   last_reset: ${group.last_reset}, last_sent: ${group.last_sent}`);
        ...
        if (group.sent_today >= group.daily_limit) {
            console.log(`🚫 Grupo ${group.group_name} atingiu limite diário: ${group.sent_today}/${group.daily_limit}`);
            return false;
        }
        ...
        if (group.last_sent) {
            ...
            console.log(`   Diferença: ${diffSeconds} segundos, min_interval: ${group.min_interval}`);

            if (diffSeconds < group.min_interval) {
                const waitSeconds = group.min_interval - diffSeconds;
                console.log(`⏸️  Aguardar: ${waitSeconds}s restantes (${Math.floor(waitSeconds / 60)} minutos)`);
                return false;
            }
        }
